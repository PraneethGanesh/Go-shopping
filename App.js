...
const addCourseToCartFunction = (PROcourse) => {
    const alreadyCourses = cartCourses
                           .find(item => item.product.id === PROcourse.id);
    if (alreadyCourses) {
        const latestCartUpdate = cartCourses.map(item =>
            item.product.id === PROcourse.id ? { 
            ...item, quantity: item.quantity + 1 } 
            : item
        );
        setCartCourses(latestCartUpdate);
    } else {
        setCartCourses([...cartCourses, {product: PROcourse, quantity: 1}]);
    }
};
